...
<nav class="navbar navbar-dark bg-dark">
  <a class="navbar-brand mb-0 h1" href="/photo/list">사진게시판</a>
 ...
 <div class="container">
		...
		<table class="table table-striped table-dark table-hover">
			...
			<c:if test="${!empty pbList }">
				<c:forEach items="${pbList}" var="pb">
					...
					<tr onclick="location.href='/detail/${pb.pbNum}'">
					


						<td><input type="checkbox" name="pbNums" value="${pb.pbNum}"></td>
						<td>${pb.pbNum}</td>
						<td><a href='<c:url value='/photo/list?pbNum=${pb.pbNum}'/>' class="text">${pb.pbTitle}</a></td> 
						<td>
	...
	<div class="search">
		<form action="/photo/list">
			제목 : <input type="text" name="pbTitle" value="${param.pbTitle}">
			내용 : <input type="text" name="pbContent" value="${param.pbContent}">
			일자 : <input type="date" name="credat1" value="${param.credat1}"> - <input type="date" name="credat2" value="${param.credat2}">
